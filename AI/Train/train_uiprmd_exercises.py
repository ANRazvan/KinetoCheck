...
class ExercisePairDataset(Dataset):
    def __init__(self, records: list[dict], template_tensor: torch.Tensor, preprocessor: UIPRMDPreprocessor):
    ...
    def _record_to_tensor(self, record: dict, preprocessor: UIPRMDPreprocessor) -> torch.Tensor:
...
def split_by_subject(records: list[dict], train_ratio: float, val_ratio: float, test_ratio: float, seed: int) -> tuple[list[dict], list[dict], list[dict]]:
    ...
    if total == 1:
        only = subjects[0]
        subject_to_split = {only: "train"}
    else:
        raw_train = max(0.0, float(train_ratio))
        raw_val = max(0.0, float(val_ratio))
        raw_test = max(0.0, float(test_ratio))
        raw_sum = raw_train + raw_val + raw_test
        if raw_sum <= 0.0:
            raw_train, raw_val, raw_test = 0.8, 0.1, 0.1
            raw_sum = 1.0

        train_share = raw_train / raw_sum
        val_share = raw_val / raw_sum
        test_share = raw_test / raw_sum

        n_train = max(1, int(round(total * train_share)))
        n_val = max(1, int(round(total * val_share))) if total >= 3 else 0
...
def build_template_tensor(records: list[dict], preprocessor: UIPRMDPreprocessor) -> torch.Tensor:
    ...
    for record in correct_records:
        ...
        tensors.append(torch.from_numpy(np.transpose(features, (2, 0, 1)).copy()).float())

    template_tensor = torch.stack(tensors, dim=0).mean(dim=0)
...
def search_best_threshold(scores: torch.Tensor, targets: torch.Tensor) -> tuple[float, dict[str, float]]:
    ...
    if math.isclose(min_score, max_score):
        threshold = min_score
        metrics = classification_metrics(scores, targets, threshold)
...
def train_one_exercise(exercise_id: int, cfg: TrainingConfig) -> dict[str, float]:
    ...
    val_dataset = ExercisePairDataset(val_records if val_records else train_records, template_tensor, preprocessor)
    test_dataset = ExercisePairDataset(test_records if test_records else val_records if val_records else train_records, template_tensor, preprocessor)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train one UI-PRMD ST-GAT model per exercise.")
